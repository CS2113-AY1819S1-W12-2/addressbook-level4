...
* *Sort* : `sort s/ATTRIBUTE [o/ORDER]` +
e.g. `sort s/name` or `sort s/kpi o/des`
...
Sorts the address book by the respective attributes specified alphabetically and/or numerically. +
Format: `sort s/ATTRIBUTE [o/ORDER]`

[TIP]
`ATTRIBUTE` refers to the name, phone, email, address, position and KPI of the contacts in the address book.

* You must key one of the following for `ATTRIBUTE`:
** `name`
** `phone`
** `email`
** `address`
...
[TIP]
`ORDER` refers to the sorting the address book in ascending or descending order.

* You can key one of the following for `ORDER`:
** Ascending order:
**** `asc`
** Descending order:
**** `des`
...
[TIP]
By default, the sort command sorts the address book according to the attribute in ascending order if no `ORDER` is specified.

Examples:

* `sort s/name` +
Sorts the address book in ascending order according to the names of the contacts.

image::sort_name_example1.png[width="400"]

* `sort s/name o/asc` +
Sorts the address book in ascending order according to the names of the contacts.

image::sort_name_example2.png[width="400"]

* `sort s/phone o/des` +
Sorts the address book in descending order according to the phone number of the contacts.

image::sort_phone_example1.png[width="400"]
